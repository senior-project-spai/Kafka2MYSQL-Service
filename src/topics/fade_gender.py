...
def handler(msg):
    ...
    for _, result in msg_dict["detail"].items():
        ...
        if face_to_update is not None:
            ...
            query = ("INSERT INTO face (id, image_id, gender_id, position_top, position_right, position_bottom, position_left, timestamp) "
                     "VALUES (%(id)s, %(image_id)s, %(gender_id)s, %(position_top)s, %(position_right)s, %(position_bottom)s, %(position_left)s), %(timestamp)s")
